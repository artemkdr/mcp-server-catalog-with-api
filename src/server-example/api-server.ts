...
app.get('/api/v1/products', (c) => {
    ...
    // Apply filters
    if (category) {
        filteredProducts = filteredProducts.filter((p) => p.category === category || p.subcategory === category);
    }
...
app.get('/api/v1/products/:id/availability', (c) => {
    ...
    const availability = {
        ...
        availability: product.inStock ? (product.stockQuantity > 10 ? 'in_stock' : 'limited_stock') : 'out_of_stock',
...
app.get('/api/v1/categories', (c) => {
    ...
    if (includeProductCount) {
        result = result.map((category) => ({
            ...category,
            productCount: products.filter((p) => p.category === category.id || p.subcategory === category.id).length,
        }));
    }
...
app.get('/api/v1/categories/:id', (c) => {
    ...
    const productCount = products.filter((p) => p.category === categoryId || p.subcategory === categoryId).length;
...
app.get('/api/v1/categories/:id/products', (c) => {
    ...
    const categoryProducts = products.filter((p) => p.category === categoryId || p.subcategory === categoryId);
